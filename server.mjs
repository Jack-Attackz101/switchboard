#!/usr/bin/env node
/**
 * SWITCHBOARD — local server
 *
 * Zero dependencies. Node 18+. Run:  node server.mjs
 *
 * It does four things:
 *   1. serves the chat UI (index.html, sitting next to this file)
 ...
 *   slack — reaches bots that only live in Slack, by posting and reading the thread
 *   aside — reads your real Aside memory off disk for context
...
server.listen(PORT, () => {
  console.log(`\n  SWITCHBOARD  →  http://localhost:${PORT}\n`);
  for (const a of CONFIG.agents) {
